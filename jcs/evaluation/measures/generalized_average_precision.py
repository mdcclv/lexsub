...
class GeneralizedAveragePrecision(object):
    ...
    def calc(gold_vector, evaluated_vector, random=False):
        ...
        for [key, value] in gold_vector:
            gold_map[key] = value
        sorted_gold_vector = sorted(
            gold_vector, key=itemgetter(1), reverse=True)
        gold_vector_accumulated = GeneralizedAveragePrecision.accumulate_score(
            sorted_gold_vector)

        ''' first we use the eval score to sort the eval vector accordingly '''
        if random is False:
            sorted_evaluated_vector = sorted(
                evaluated_vector, key=itemgetter(1), reverse=True)
        else:
        ...
        for (key, accum_score) in evaluated_vector_accumulated:
            i += 1
            if (key in list(gold_map.keys())) and (gold_map.get(key) > 0):
    ...
    def calcTopN(gold_vector, evaluated_vector, n, measure_type):
        ...
        for [key, value] in gold_vector:
            gold_map[key] = value
        gold_vector_sorted = sorted(
            gold_vector, key=itemgetter(1), reverse=True)
        gold_top_score_sum = sum([float(score)
                                  for (key, score) in gold_vector_sorted[0:n]])
