...
to_wordnet_pos = {'N': wordnet.NOUN, 'J': wordnet.ADJ,
                  'V': wordnet.VERB, 'R': wordnet.ADV}
from_wordnet_pos = {wordnet.NOUN: 'N', wordnet.ADJ: 'J',
                    wordnet.VERB: 'V', wordnet.ADV: 'R'}
...
def is_atomic_mwe(mwe, verb_lemma, complement_lemma, synsets):
    ...
    for synset in synsets:
        gloss_lemmas = set([WordNetLemmatizer().lemmatize(word)
...
def detect_mwe(text_tokens, target_ind, wordnet_pos):
    if (target_ind < len(text_tokens)-1):
        verb_lemma = WordNetLemmatizer().lemmatize(
            text_tokens[target_ind], wordnet_pos)
        complement_lemma = WordNetLemmatizer().lemmatize(
            text_tokens[target_ind+1])
        mwe = '_'.join([verb_lemma, complement_lemma])
        synsets = wordnet.synsets(mwe, wordnet.VERB)
        if len(synsets) > 0:
            if (target_ind+1 < len(text_tokens)-1):
                mwe_right = '_'.join([WordNetLemmatizer().lemmatize(
                    text_tokens[target_ind+1]), WordNetLemmatizer().lemmatize(text_tokens[target_ind+2])])
                if len(wordnet.synsets(mwe_right)) > 0:
                    return
            if is_atomic_mwe(mwe, verb_lemma, complement_lemma, synsets):
                mwe = '='.join(
                    [text_tokens[target_ind], text_tokens[target_ind+1]])
