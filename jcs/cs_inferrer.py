...
default_generated_results = ['time', 'people', 'information',
    ...
    def generate_inferred(self, result_vec, target_word, target_lemma, pos):
        ...
        if result_vec is not None:
            for word, weight in result_vec:
                # make sure this is not junk
                if generated_word_re.match(word) is not None:
            ...
            for word, weight in result_vec:
                ...
                if lemma in candidates:
                    self.add_inference_result(
                        lemma, weight, filtered_results, candidates_found)
                if lemma.title() in candidates:
                    self.add_inference_result(
                        lemma.title(), weight, filtered_results, candidates_found)
                if word in candidates:
                    # there are some few cases where the candidates are not lemmatized
                    self.add_inference_result(
                        word, weight, filtered_results, candidates_found)
                if word.title() in candidates:
                    # there are some few cases where the candidates are not lemmatized
                    self.add_inference_result(
                        word.title(), weight, filtered_results, candidates_found)
