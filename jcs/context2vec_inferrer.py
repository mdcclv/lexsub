...
class Context2vecInferrer(CsInferrer):
    ...
    def represent_target_and_context(self, lst_instance, tfo):
        ...
        # if there's only the target word in the sentence then we don't ignore it...
        ignore_target = self.ignore_target and len(sent_words) > 1

        if not ignore_target:
            if lst_instance.target not in self.word2index:
                tfo.write(
                    "ERROR: %s not in word embeddings.Trying lemma.\n" % lst_instance.target)
        ...
        target_v = self.target_words[self.word2index[sent_words[target_ind]]
                                     ] if not ignore_target else None
