...
class ContextInstance(object):
    ...
    def __init__(self, line, no_pos_flag):
        ...
        self.target_key = '.'.join(self.full_target_key.split(
            '.')[:2])  # remove suffix in cases of bar.n.v
        ...
        self.target_pos = '.'.join(
            [self.target, '*']) if no_pos_flag is True else '.'.join([self.target, self.pos])
    ...
    def get_neighbors(self, window_size):
        ...
        neighbors = tokens[start_pos:self.target_ind] + \
            tokens[self.target_ind+1:end_pos]
        return neighbors
