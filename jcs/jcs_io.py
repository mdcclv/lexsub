...
def vec_to_str(subvec, max_n):
    ...
    sub_strs = [' '.join([word, wf2ws(weight)])
                for word, weight in sub_list_sorted]
...
def get_pmi_weights(subvec, w2counts, sum_counts, offset, threshold, normalize=False):
    ...
    if normalize:
        norm = norm**0.5
        for i in range(0, len(subvec_pmi)):
